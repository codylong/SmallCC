...
class CC(gym.Env):
	...
	def second_init(self): # after nmod, sigma, eps are set, can run this
		...
		rounded_down_evecs = [[int(math.floor(entry)) for entry in ev] for ev in new_eig_vecs]
		rounded_up_evecs = [[int(math.ceil(entry)) for entry in ev] for ev in new_eig_vecs]

		origin_prempm = rounded_down_evecs[np.argmin(eig_vals)]
		print self.origin
		if self.origin == None:
		...
		#rd = [mpf(a) for a in rounded_down_evecs]
		#ru = [mpf(a) for a in rounded_up_evecs] 
		#print "origin \n", self.origin
		...
		self.action_space = spaces.Discrete(self.nmod)
		self.observation_space = spaces.Box(low=0, high=1, shape=(self.nmod, 1))
