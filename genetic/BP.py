...
class IndividualsClass:
    def __init__(self, state, ngvec):
        self.state = state
        self.ngvec = ngvec
...
L0 = EPS
METRIC = np.array([[]])
NGVEC = np.array([])  # keep track of the NG vector to speed up the computation
ORIGIN_CC = mpf(0)
...
def evaluate_cc(individual):
    global ORIGIN_CC
    avec = individual[0].state
    cc = ORIGIN_CC + np.dot(np.dot(METRIC, avec), avec)
    d = dist(cc)
    return -np.float(d),
...
# Attribute generator
toolbox.register("attr_item", IndividualsClass, [random.randint(-NMOD, NMOD) for _ in range(NMOD)], list(NGVEC))
