...
def main():
    ...
    init_metric()
    # print EIGVALS
    prob = LpProblem("The CC Problem", LpMinimize)
    vars = []
    for i in range(NMOD):
        vars.append(LpVariable(str(i), None, None, LpInteger))

    prob += BARE_CC + lpSum([vars[i] * EIGVALS[i] for i in range(NMOD)]), "Obj"
    prob += BARE_CC + lpSum([vars[i] * EIGVALS[i] for i in range(NMOD)]) >= EPS, "Shell bound"

    prob.writeLP("CCExample.lp")
    prob.solve()
    # print "Status:", LpStatus[prob.status]
    # for v in prob.variables():
    #     print v.name, "=", v.varValue

    # print "Minimal CC = ", sum([mpf((prob.variables()[i]).varValue)*EIGVALS[i] for i in range(NMOD)])
