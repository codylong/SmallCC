...
def writeScript(eps,nmod,sigma,max_steps,personstring,beta,power,gam,arch):
    ...
    out += "\nmpirun -prot -srun -n 1 python train_a3c_gym.py 32 --steps 20000000000 --env cc-inout2-v0 --outdir /gss_gpfs_scratch/" + personstring + "/wishart/"+outdir + " --gamma " + str(gam)+ " --eps " + str(eps) + " --nmod " + str(nmod) + " --sigma " + str(sigma) + " --beta " + str(beta)+ " --eval-interval 2000000 --reward-d-pow " + str(power) + " --arch " + arch
            
    f = open("/home/" + personstring + "/wishart/scripts/e"+str(eps)+"k"+str(nmod)+"pow"+str(power)+"beta"+str(beta)+"s"+str(sigma)+"msteps"+max_steps+"arch"+arch+".job",'w')
    f.write(out)
    f.close()
    output=subprocess.Popen("sbatch /home/" + personstring +  "/wishart/scripts/e"+str(eps)+"k"+str(nmod)+"pow"+str(power)+"beta"+str(beta)+"s"+str(sigma)+"msteps"+max_steps+"arch"+arch+".job",shell=True,stdout=subprocess.PIPE).communicate()[0]
    return output
