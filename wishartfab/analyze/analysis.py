...
class Wishart_Experiment:
    ...
    def __init__(self,dir):
        ...
        self.p, self.s = {'worker': [], 'time': [], 'cc':  []}, {'worker': [], 'time': [], 'cc': []}
    ...
    def read_command_txt(self):
        ...
        if 'home' in self.outdir: self.outdir = self.outdir.replace("home","gss_gpfs_scratch") # in case not using scratch
    ...
    def create_job_file(self, submit_string, home_dir, partition = "ser-par-10g-5"):
        ...
        s += "\ncd " + home_dir
        s += "\nmpirun -prot -srun -n 1 python " + submit_string
        s += "\n"
...
class Wishart_Experiments:
    ...
    def __init__(self,parent_dir,discovery_dir="/home/jhhalverson/wishart/",respect_ignores = True, respect_day = False): 
        ...
        self.exps = []
        for d in dirs:
            if os.path.isdir(parent_dir+d) and d[:2] == '20' and (respect_ignores == False or not os.path.isfile(parent_dir+d+"/ignore.txt")) and (respect_day == False or (now-datetime.datetime.strptime(d,'%Y%m%dT%H%M%S.%f')).days >= 1):
                print parent_dir + d
    ...
    def get_cont_classes(self):
        ...
        for eq in self.eq_classes:
            if len(eq.exps) > 1 or eq.exps[0].nmod == 250:
                to_cont.append(eq)
        return to_cont
