...
def train_loop(process_idx, env, agent, steps, outdir, counter,
    ...
    if eval_env is None:
        eval_env = env
        eval_env.env.set_output_file_path(outdir)
        eval_env.env.set_process_idx(process_idx)
    ...
    try:
        ...
        episode_r = 0
        global_t = 0
        env.env.set_global_t(global_t)
        eval_env.env.set_global_t(global_t)
        local_t = 0
        global_episodes = 0
        obs = env.reset()
        r = 0
        ...
        while True:
            ...
            if done or episode_len == max_episode_len:
                ...
                best_action = env.env.get_best_action_and_update_tree(agent)
                a = agent.act_and_train(obs, r, best_action)
                ...
                with counter.get_lock():
                    ...
                    env.env.set_global_t(global_t) # FR: update global t counter in  the workers
...
def train_agent_async(outdir, processes, make_env,
    ...
    def run_func(process_idx):
        ...
        env.env.set_output_file_path(outdir)
        env.env.set_process_idx(process_idx)
        env.env.init_output()
